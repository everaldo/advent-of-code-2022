...
def left_scenic_score(map, row_index, column_index)
  return 0 if column_index == 0

  tree_height = map[row_index][column_index]
  score = 0

  (column_index - 1).downto(0).take_while do |column|
    score += 1
    tree_height > map[row_index][column]
  end
  score
end

def right_scenic_score(map, row_index, column_index)
  tree_height = map[row_index][column_index]
  size = map.length

  return 0 if column_index == size - 1

  score = 0
  ((column_index + 1)...size).take_while do |column|
    score += 1
    tree_height > map[row_index][column]
  end
  score
...
def top_scenic_score(map, row_index, column_index)
  return 0 if row_index == 0

  tree_height = map[row_index][column_index]
  score = 0

  (row_index - 1).downto(0).take_while do |row|
    score += 1
    tree_height > map[row][column_index]
  end
  score
end

def bottom_scenic_score(map, row_index, column_index)
  tree_height = map[row_index][column_index]
  size = map.length
  score = 0

  return 0 if row_index == size - 1

  ((row_index + 1)...size).take_while do |row|
    score += 1
    tree_height > map[row][column_index]
  end
  score
...
def scenic_score_for(map, row_index, column_index)
  left_scenic_score(map, row_index, column_index) *
  right_scenic_score(map, row_index, column_index) *
  top_scenic_score(map, row_index, column_index) *
  bottom_scenic_score(map, row_index, column_index)
end

def calculate_max_scenic_score(map)
  scenic_score = []
  map.each_with_index do |tree_line, row_index|
    scenic_score[row_index] = []
    tree_line.each_with_index do |tree_height, column_index|
      scenic_score[row_index][column_index] = scenic_score_for(map, row_index, column_index) 
    end
  end
  scenic_score.map { |row| row.max }.max
end
...
puts calculate_max_scenic_score(map)